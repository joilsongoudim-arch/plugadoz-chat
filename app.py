...
.content { flex: 1; overflow-y: auto; background: #111b21; }
.tab { display: none; }
...
.status-item { display: flex; flex-direction: column; align-items: center; cursor: pointer; flex-shrink: 0; width: 70px; position: relative; }
.status-ring {
...
.status-add-icon {
    position: absolute; bottom: 18px; right: 0; background: #00a884; color: white;
    width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 12px; border: 2px solid #111b21; z-index: 2;
}
...
.bottom {
    height: 60px; flex-shrink: 0; display: flex; border-top: 1px solid #222d34; background: #111b21; z-index: 10;
}
.nav {
    flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px; color: #8696a0; font-size: 11px; cursor: pointer;
}
...
.messages {
    flex: 1; overflow-y: auto; padding: 15px; display: flex; flex-direction: column; gap: 7px;
}
...
<div id="login">
    ...
    <input id="username" maxlength="40" placeholder="Digite seu nome" autocomplete="off" value="Teste">
    <button type="button" onclick="entrarApp()">Entrar</button>
</div>
...
<div id="app">
    <div class="header">
        ...
        <div class="header-actions">
            <button onclick="abrirPerfil()">⚙️</button>
            <button onclick="novoGrupo()">＋</button>
        </div>
    </div>
    ...
    <div class="content">
            ...
            <div class="updates-section">
                ...
                <div class="status-tray">
                    <div class="status-item">
                        <div class="status-ring my-status" onclick="verMeuStatus()">
                            <div class="status-avatar" id="my-status-avatar">EU</div>
                        </div>
                        <div class="status-add-icon" onclick="criarStatusPrompt()">＋</div>
                        <div class="status-name">Meu status</div>
                    </div>
                </div>
            </div>
            <div class="fab-container">
                <button class="fab-small" onclick="criarStatusPrompt()">✏️</button>
                <button class="fab-large" onclick="document.getElementById('status-file-input').click()">📷</button>
        ...
        <div id="comunidades" class="tab">
            <div class="empty"><h3>Comunidades</h3></div>
        </div>

        <div id="ligacoes" class="tab">
            <div class="empty"><h3>Ligações</h3></div>
        </div>
    </div>
...
<div id="status-viewer">
    ...
    <div class="status-content-area" id="viewer-content"></div>
</div>
...
<div id="chat-screen">
    ...
    <div class="chat-footer">
        ...
        <button class="action-btn" id="btn-main" 
                ...
                ontouchstart="iniciarToque(event)" ontouchend="pararToque(event)"
                onmousedown="iniciarToque(event)" onmouseup="pararToque(event)">🎤</button>
...
let meuUltimoStatus = { tipo: 'texto', conteudo: 'Você ainda não publicou nenhum status!' };

function entrarApp() {
    const input = document.getElementById("username");
    const nome = input.value.trim();
    if (nome) { meuNome = nome; }
    
    // Força total na marra para sumir o login e mostrar o app com os ícones
    document.getElementById("login").style.display = "none";
    document.getElementById("app").style.display = "flex";
    
    const avatarEl = document.getElementById("my-status-avatar");
    if (avatarEl) {
        avatarEl.innerText = meuNome.charAt(0).toUpperCase();
    }
}
...
function abrirChat(nome) {
    ...
    if (socket) socket.emit("join", { room: salaAtual });
}

function fecharChat() {
    if (salaAtual && socket) socket.emit("leave", { room: salaAtual });
    salaAtual = "";
    document.getElementById("chat-screen").style.display = "none";
}

function criarStatusPrompt() {
    const texto = prompt("Digite o texto do seu status:");
    if (texto && texto.trim()) {
        meuUltimoStatus = { tipo: 'texto', conteudo: texto.trim() };
        alert("Status publicado com sucesso!");
    }
}

function publicarStatusFoto(input) {
    const file = input.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = function(e) {
        meuUltimoStatus = { tipo: 'foto', conteudo: e.target.result };
        document.getElementById("my-status-avatar").innerHTML = `<img src="${e.target.result}">`;
...
function verMeuStatus() {
    const contentArea = document.getElementById("viewer-content");
    document.getElementById("viewer-name").innerText = "Você (Meu status)";
    document.getElementById("viewer-avatar").innerText = meuNome ? meuNome.charAt(0) : "EU";
    
    if (meuUltimoStatus.tipo === 'foto') {
        contentArea.innerHTML = `<img src="${meuUltimoStatus.conteudo}">`;
    } else {
        contentArea.innerHTML = `<div class="status-content-text">${meuUltimoStatus.conteudo}</div>`;
    }
    ...
    statusTimeout = setTimeout(() => { fecharStatusViewer(); }, 5000);
}
...
function mudarBotaoEnvio() {
    ...
    if (texto.length > 0) { btn.innerHTML = "➤"; } else { btn.innerHTML = "🎤"; }
}

function cliqueBotaoPrincipal() {
    const texto = document.getElementById("message").value.trim();
    if (texto.length > 0) { enviarTexto(); }
}
...
function enviarTexto() {
    ...
    const payload = { room: salaAtual, username: meuNome, type: "text", content: texto };
    if (socket) socket.emit("message", payload); else appendMessage(payload);
    input.value = "";
    mudarBotaoEnvio();
    input.focus();
}
...
function enviarArquivo(input) {
    ...
    const reader = new FileReader();
    reader.onload = function(e) {
        const payload = { room: salaAtual, username: meuNome, type: "image", content: e.target.result };
        if (socket) socket.emit("message", payload); else appendMessage(payload);
    };
    reader.readAsDataURL(file);
    input.value = "";
...
function iniciarToque(e) {
    if (document.getElementById("message").value.trim().length > 0) return;
    gravando = true;
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
        ...
        mediaRecorder.onstop = () => {
            ...
            reader.onload = function(ev) {
                const payload = { room: salaAtual, username: meuNome, type: "audio", content: ev.target.result };
                if (socket) socket.emit("message", payload); else appendMessage(payload);
            };
        ...
        timerInterval = setInterval(() => {
            ...
            document.getElementById("rec-timer").innerText = Math.floor(diff/60) + ":" + (diff%60 < 10 ? "0" : "") + (diff%60);
        }, 1000);
    }).catch(() => {});
}
