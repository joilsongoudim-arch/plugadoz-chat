...
socketio = SocketIO(
    ...
    max_http_buffer_size=10 * 1024 * 1024  # Permite envio de imagens/áudios maiores
)
...
def handle_join(data):
    ...
    if room:
        ...
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("SELECT username, type, content FROM messages WHERE room = ? ORDER BY id ASC LIMIT 50", (room,))
...
def handle_message(data):
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO messages (room, username, type, content) VALUES (?, ?, ?, ?)", (room, username, msg_type, content))
    conn.commit()
    conn.close()
